...
#   - Secret "libretto-benchmarks-kernel-api-key" already exists in Secret Manager
#
# Usage:
...
if gcloud run jobs describe "${JOB_NAME}" --region="${REGION}" --quiet 2>/dev/null; then
  ...
  gcloud run jobs update "${JOB_NAME}" \
    ...
    --cpu=4 \
    --memory=8Gi \
    --set-secrets=ANTHROPIC_API_KEY=anthropic-api-key:latest,KERNEL_API_KEY=libretto-benchmarks-kernel-api-key:latest \
    --quiet
else
