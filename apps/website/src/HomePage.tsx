...
function WarningTriangle({ className = "" }: { className?: string }) {
  return (
    <svg width="14" height="14" fill="none" viewBox="0 0 24 24" className={className}>
...
function SectionDivider({ caution }: { caution?: boolean } = {}) {
  return (
    <div
    ...
    >
      {caution && (
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="flex items-center gap-1 border border-rule bg-bg px-1 py-1 font-mono text-[9px] uppercase leading-none tracking-widest text-faint">
            <WarningTriangle className="size-[10px]" />
            Caution
            <WarningTriangle className="size-[10px]" />
          </span>
        </div>
