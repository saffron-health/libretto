...
Always declare `startUrl` on every workflow. It is the entry URL Libretto opens before the handler runs. On Kernel and other cloud providers, Libretto also sends it at browser create time so the first navigation happens before CDP attach.
...
export default workflow(
  ...
  async (ctx, input) => {
    // Browser is already on startUrl — do not page.goto the same URL here.
  },
);
```
