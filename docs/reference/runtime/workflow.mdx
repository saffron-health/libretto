...
These fields are stored in deploy metadata. Hosted jobs and schedules inherit
them from the deployed workflow. Override them for a single run with
`libretto cloud jobs create --start-url ... --gpu --viewport WxH` (or the
matching schedule flags).
