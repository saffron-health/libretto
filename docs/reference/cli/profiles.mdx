...
```bash theme={null}
google-chrome \
  --user-data-dir="$HOME/.libretto/browser-profiles/my-account" \
  --remote-debugging-port=9222
```

After signing in through that Chrome window, connect Libretto:

```bash theme={null}
npx libretto connect http://127.0.0.1:9222 --session my-account
```

Later runs must start Chrome with the same user-data directory. This pattern
works on your machine or a self-hosted worker whose filesystem and browser
lifecycle you control. For hosted browsers, use the provider's native profile
feature instead.
