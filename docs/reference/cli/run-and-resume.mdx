...
```bash theme={null}
libretto run ./integration.ts
libretto run ./integration.ts --headless
```

#### Syntax

```bash theme={null}
libretto run <file> [flags]
```
...
<ParamField path="--auth-profile" type="string">
  Domain of a saved auth profile to load for this run. Example: `--auth-profile linkedin.com`. Profiles are created with `libretto save <domain> --session <name>`.
</ParamField>
...
<Info>
  ...
  providers](/alternative-providers/overview) for Kernel, Browserbase, AWS, and
  GCP setup.
</Info>
...
libretto run ./integration.ts --session debug-flow --headed

# Inspect the failure state
libretto snapshot --session debug-flow

# Prototype a fix
libretto exec --session debug-flow "await page.locator('.error-message').textContent()"
...
libretto run ./integration.ts --headed
```
...
libretto run ./integration.ts --headless --params '{"status":"open"}'

# Run with a params file
libretto run ./integration.ts --params-file ./params.json

# Run with a saved auth profile
libretto run ./integration.ts --auth-profile linkedin.com
...
<Steps>
  ...
  <Step title="Inspect the paused page state">
    ...
    libretto exec --session debug-flow "await page.url()"
    ```
  ...
  <Step title="Resume the workflow">
    ```bash  theme={null}
    libretto resume --session debug-flow
    ```
