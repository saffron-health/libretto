...
```bash theme={null}
libretto exec --session debug-example "await page.url()"
```
...
<Tabs>
  <Tab title="Single-line">
    ...
    ```bash  theme={null}
    libretto exec --session debug-example "await page.url()"
    libretto exec --session debug-example "await page.locator('button').count()"
    libretto exec --session debug-example "await page.locator('button:has-text(\"Continue\")').click()"
    ```
  ...
  <Tab title="stdin (multi-line)">
    ...
    ```bash  theme={null}
    echo "await page.url()" | libretto exec - --session debug-example
    ```

    ```bash  theme={null}
    cat <<'EOF' | libretto exec - --session debug-example
    ...
    libretto exec --session debug-example "await tableRows('table')"
    ```
...
<ParamField path="--page" type="string">
  Target a specific page ID within the session. Use `libretto pages --session <name>` to list page IDs when a popup or new tab is open.
</ParamField>
...
```bash theme={null}
libretto exec --session debug-example "await page.title()"
# My Page Title

libretto exec --session debug-example "await page.locator('li').count()"
# 12
```
...
libretto exec --session debug-example "await page.locator('input[name=\"email\"]').inputValue()"
