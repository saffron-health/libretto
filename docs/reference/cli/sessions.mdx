...
```bash theme={null}
libretto open https://example.com --read-only --session diag
libretto connect ws://localhost:9222 --read-only --session remote-diag
libretto run ./workflow.ts --read-only --session test-run
```
...
# Switch to read-only
libretto session-mode read-only --session my-session

# Switch back to write-access
libretto session-mode write-access --session my-session
```
...
```bash theme={null}
# Read page title
libretto readonly-exec --session diag "return await page.title()"

# Check if an element is visible
libretto readonly-exec --session diag "return await page.locator('#submit').isVisible()"

# Get text content of a section
libretto readonly-exec --session diag "return await page.locator('.results').textContent()"
```
