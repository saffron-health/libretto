...
<ParamField path="--provider, -p" type="string">
  Browser provider to use. One of `local`, `kernel`, `browserbase`, `steel`, or `libretto-cloud`. Defaults to `local`.
...
# Open with Libretto Cloud
export LIBRETTO_API_KEY="your-key"
npx libretto open https://example.com --provider libretto-cloud
...
npx libretto snapshot --session chrome-debug

# Run code against it
npx libretto exec --session chrome-debug "await page.url()"
```

#### Connect to a persistent browser profile

Saved auth profiles restore cookies, localStorage, and IndexedDB into a new
browser context. Some websites also depend on sessionStorage or bind the session
to a specific browser process. For those websites, start Chrome with a dedicated
user-data directory and attach Libretto to the running browser:
...
npx libretto snapshot --session my-account
```

Later runs must start Chrome with the same `--user-data-dir`. Only one browser
process can use a user-data directory at a time. Store the directory on
persistent, access-controlled disk and use a separate directory for each
account.

This pattern works when Chrome runs on your machine or on a self-hosted worker
whose filesystem and browser lifecycle you control. Hosted browser providers do
not expose their user-data directory through Libretto; use the provider's native
persistent-profile feature instead.

<Note>
  The Chrome executable name and location vary by operating system. Recent
  Chrome versions require a non-default user-data directory when remote
