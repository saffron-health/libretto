...
libretto open https://example.com --viewport 1440x900 --session my-session

# Open for a manual login flow, then save the session
libretto open https://linkedin.com --headed --session app-login
# ... user logs in ...
libretto save linkedin.com --session app-login
```
...
libretto open https://example.com -p browserbase
...
libretto open https://example.com --provider libretto-cloud
...
libretto connect ws://127.0.0.1:9223 --session another-session
```
...
<Warning>
  Libretto does **not** manage the lifecycle of a connected process. Running `libretto close --session <name>` clears the Libretto session record but does **not** terminate the remote browser or Electron app.
</Warning>
...
libretto connect http://127.0.0.1:9222 --session chrome-debug

# Inspect the attached page
libretto snapshot --session chrome-debug

# Run code against it
libretto exec --session chrome-debug "await page.url()"
```
