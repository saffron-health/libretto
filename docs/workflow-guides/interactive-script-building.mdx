---
title: Interactive script building
---
...
<Steps>
  ...
  <Step title="Perform the workflow in the browser">
    ...
    <Note>
      Take your time. The recording captures everything, so there's no penalty for pausing to think or scrolling around to orient yourself. If you make a mistake, just undo it naturally — the agent can filter out dead ends.
    </Note>
  ...
  <Step title="Wait for the agent to recreate your workflow">
    ...
    It uses this log as a blueprint to write a TypeScript workflow file that replicates your steps programmatically. The agent may also snapshot the page or test selectors against the live DOM to make sure everything works. You don't need to do anything during this phase.

    If the agent asks clarifying questions about what you did, answer them — but usually the action log plus your summary is enough.
