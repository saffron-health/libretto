...
```text
Plan:    Pro (active)
Usage:   12.40 / 30 browser hours this period
...
A session enters standby a few seconds after the last Chrome DevTools Protocol or Live View client disconnects. Once it does, the session's inactivity timeout begins counting down toward automatic teardown. Reconnecting any client wakes the session immediately and metering resumes.
