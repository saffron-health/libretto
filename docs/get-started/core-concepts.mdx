...
**`config.json`:** Your AI provider and viewport settings. Created by `npm init libretto@latest` and updated with `npx libretto ai configure`. See [Configuration](./configuration) for details.
