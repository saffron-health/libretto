...
<Tip>
  **Recommended approach:** Use in-browser fetch (`pageRequest()`) for most sites. For high-security sites with aggressive bot detection, combine Regular Playwright navigation with passive `page.on('response')` interception instead.
</Tip>
...
<Tip>
  Use in-browser fetch (`pageRequest()`) for most sites. It gives you full control over which endpoints you call, structured JSON responses, and the real browser's network fingerprint.
