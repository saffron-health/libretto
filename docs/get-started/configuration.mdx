...
<ParamField path="ai.model" type="string">
  Optional. A `provider/model-id` string (e.g. `openai/gpt-5.4`). Only needed if you want to use `npx libretto snapshot`. Without this, snapshot will fail but all other commands work fine.
</ParamField>
