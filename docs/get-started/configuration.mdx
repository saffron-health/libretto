...
`npm init libretto@latest` handles all initial configuration for you. The commands on this page are for changing settings after the fact.
...
<ParamField path="ai.model" type="string" required>
  A `provider/model-id` string (e.g. `openai/gpt-5.4`). Required for `npx libretto snapshot` to work. Without this, snapshot analysis will fail.
</ParamField>
