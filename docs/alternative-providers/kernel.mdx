...
title: Kernel
sidebarTitle: Kernel
"og:image": "https://libretto.sh/docs/og/alternative-providers/kernel.png"
---
...
When the workflow declares `startUrl`, Libretto passes it to Kernel as `start_url` at create time and does not call `page.goto` after CDP attach. Declare `gpu` and `viewport` on the workflow the same way when those sites need them. See [Workflow browser launch options](/reference/runtime/workflow#browser-launch-options).
