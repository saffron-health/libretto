...
```bash theme={null}
libretto open https://example.com --session kernel-demo
```

If you did not set `provider` in `.libretto/config.json`, pass it for this command instead:

```bash theme={null}
libretto open https://example.com --provider kernel --session kernel-demo
```

After the session is open, use the normal Libretto commands:

```bash theme={null}
libretto snapshot --session kernel-demo
libretto exec --session kernel-demo "await page.title()"
libretto close --session kernel-demo
```
...
```bash theme={null}
libretto run ./workflow.ts --provider kernel --headless
```
