...
```bash theme={null}
libretto open https://example.com --session browserbase-demo
```

If you did not set `provider` in `.libretto/config.json`, pass it for this command instead:

```bash theme={null}
libretto open https://example.com --provider browserbase --session browserbase-demo
```

After the session is open, use the normal Libretto commands:

```bash theme={null}
libretto snapshot --session browserbase-demo
libretto exec --session browserbase-demo "await page.title()"
libretto close --session browserbase-demo
```
...
```bash theme={null}
libretto run ./workflow.ts --headless
```

Or pass the provider for a single run:

```bash theme={null}
libretto run ./workflow.ts --provider browserbase --headless
```
