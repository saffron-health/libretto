...
title: Deploy on GCP
sidebarTitle: GCP
"og:image": "https://libretto.sh/docs/og/alternative-providers/gcp.png"
---
...
<Steps>
  <Step title="Write a dispatcher entry point">
...
```
  ...
  <Step title="Define the Cloud Run Job">
...
The `serviceAccountName` is what grants the job permission to read secrets, write to GCS, and so on. Scope it tightly.
  ...
  <Step title="Build and deploy via Cloud Build">
...
<ul>
  <li><strong>Logs:</strong> <code>gcloud logs read browser-agent-job --region=us-central1</code></li>
  <li><strong>Execution history:</strong> <code>gcloud run jobs executions list --job=browser-agent-job --region=us-central1</code></li>
  <li><strong>Session logs:</strong> upload <code>.libretto/sessions/&lt;name&gt;/logs.jsonl</code> to GCS at the end of each run if you want to keep them beyond the job's lifetime.</li>
</ul>
...
<Tip>
  Libretto writes detailed network, action, and snapshot logs under `.libretto/sessions/<name>/`. Pushing that directory to GCS on failure gives you a reproducible record for debugging. See [debugging workflows](/guides/debugging-workflows) for what to look at.
</Tip>
