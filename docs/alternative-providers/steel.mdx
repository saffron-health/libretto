...
```bash theme={null}
libretto open https://example.com --session steel-demo
```

If you did not set `provider` in `.libretto/config.json`, pass it for this command instead:

```bash theme={null}
libretto open https://example.com --provider steel --session steel-demo
```

After the session is open, use the normal Libretto commands:

```bash theme={null}
libretto snapshot --session steel-demo
libretto exec --session steel-demo "await page.title()"
libretto close --session steel-demo
```
...
```bash theme={null}
libretto run ./workflow.ts --provider steel --headless
```
