...
This page is about signing into the websites your workflows automate. For signing into Libretto Cloud itself, see [Libretto Cloud authentication](/libretto-cloud-hosting/authentication). For API key authentication to the hosted API, see [API authentication](/libretto-cloud-api/authentication).
...
If `refresh: true` is set, the hosted provider should persist browser state changes made during the run back to that profile. This lets a successful fallback login repair an expired profile for future runs.
...
The two are intentionally separate. To seed a hosted profile, run the workflow with Libretto Cloud and let fallback login create or refresh the provider-native profile.
