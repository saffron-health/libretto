...
```bash theme={null}
libretto cloud billing portal
```

Because this opens an interactive browser flow, `billing portal` requires a saved login session from `libretto cloud auth login` or `libretto cloud auth signup`. API keys can be used for `billing status`, but not for opening the portal.
...
- If `billing portal` fails with an auth error, log in again with `libretto cloud auth login`.
- If `billing status` fails with an auth error, log in again or set `LIBRETTO_API_KEY`.
