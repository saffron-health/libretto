...
<ParamField path="authProfile" type="string | false">
  Named browser state for signed-in sessions. On supported providers, omit to use `default`, or pass `false` for a session with no saved state. See [Auth profiles](/browser-tools/auth-profiles).
</ParamField>
...
<ParamField path="timeoutMs" type="number">
  Max wall-clock time to wait for this exec in milliseconds. Defaults to
  `10000` (10 seconds). A timeout returns `{ ok: false, error }` and stops
