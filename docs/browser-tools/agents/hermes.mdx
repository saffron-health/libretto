...
Local sessions keep signed-in browser state in [auth profiles](/browser-tools/auth-profiles) under `~/.libretto/browser-tools/profiles/` on the Hermes host.
...
- [OpenClaw](/browser-tools/agents/openclaw) — same binary on OpenClaw
