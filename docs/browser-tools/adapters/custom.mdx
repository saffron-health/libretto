...
Always expose `dispose()` from the host so leftover sessions close when the agent loop ends. For snapshot screenshots, the base tool returns `{ base64, mimeType: "image/png" }` — convert that into your framework's multimodal content type if needed (see the [AI SDK](/browser-tools/adapters/ai-sdk) and [Pi](/browser-tools/adapters/pi) adapters).
