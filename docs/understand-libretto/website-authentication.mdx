...
```bash theme={null}
export LIBRETTO_CLOUD_USERNAME="alice@example.com"
export LIBRETTO_CLOUD_PASSWORD="secret"
npx libretto cloud credentials push portal
```

The command stores a hosted credential named `portal`. Environment variables
with the `LIBRETTO_CLOUD_` prefix become lowercase fields with the prefix
removed, so `LIBRETTO_CLOUD_USERNAME` becomes `username` and
`LIBRETTO_CLOUD_PASSWORD` becomes `password`.
...
browser session. Set `persistAfterRun: true` when a successful local run should
