...
<Steps>
  <Step title="Open the site in headed mode">
    ```bash  theme={null}
    libretto open https://portal.example.com --headed --session portal-login
    ```
  ...
  <Step title="Save the session as a profile">
    ```bash  theme={null}
    libretto save portal.example.com --session portal-login
    ```
  ...
  <Step title="Reuse the profile in future runs">
    ```bash  theme={null}
    libretto run ./report.ts --auth-profile portal.example.com
    ```
