...
authProfile: {
  ...
  sites: ["portal.example.com", "auth.example.com"],
  refresh: true,
}
```

Use `sites` when the login spans multiple domains. Set `refresh: true` when a
successful run should save the updated browser state back to the profile.
...
profile API; local profile files are not uploaded.
