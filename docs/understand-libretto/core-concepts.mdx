...
- **CLI** (`libretto <command>`): the primary interface for both agents and humans. Commands open browsers, take snapshots, execute Playwright code, run workflow files, and manage sessions. Every command accepts `--session <name>` to target a specific browser session.
- **Library API** (`import { workflow } from "libretto"`): used inside workflow files you run with `libretto run`. The `workflow()` function wraps your automation handler and gives it typed access to `page` and `session`.
...
**`config.json`:** Workspace settings such as viewport, browser provider, and session-mode defaults. Created by `npm create libretto@latest` or `libretto setup`. See [Configuration](/get-started/configuration) for details.
...
A **session** is a named browser context. When you run `libretto open https://example.com --session my-session`, Libretto launches a Chromium instance and registers it under that name. Subsequent commands (`snapshot`, `exec`, `pages`) all target that session by name.

Sessions are created automatically when you run `open`, `connect`, or `run`, and cleaned up with `libretto close`.
...
For authenticated sites, see [Website authentication](/understand-libretto/website-authentication) to learn how to pass credentials, TOTP secrets, and save reusable login profiles.
