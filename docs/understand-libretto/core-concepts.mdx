...
**`config.json`:** Workspace settings such as viewport, browser provider, and session-mode defaults. Created by `npm create libretto@latest` or `npx libretto setup`. See [Configuration](/get-started/configuration) for details.
