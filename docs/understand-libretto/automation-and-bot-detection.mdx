...
- **Fetch patching check.** Run `libretto exec` to evaluate `window.fetch.toString()` in the browser console. If it returns actual JavaScript (not `"[native code]"`), the site monkey-patches fetch and you should prefer passive interception over in-browser fetch.
...
- **Snapshot inspection.** Use `libretto snapshot` to check for challenge pages, CAPTCHAs, or interstitials that indicate detection.
