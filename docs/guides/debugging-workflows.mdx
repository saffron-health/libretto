...
<Steps>
  <Step title="Connect to the remote browser">
    ...
    ```bash  theme={null}
    libretto connect <cdp-endpoint-url> --session remote-debug
    ```
...
<Steps>
  ...
  <Step title="Wait for the agent to diagnose and fix">
    The agent snapshots the broken page state, identifies what's wrong, and tests new selectors against the live DOM. You'll see it running `libretto snapshot` and `libretto exec` commands while the browser stays open.
...
The workflow halts at the `pause()` with the browser open. The agent can also drive this autonomously by adding `pause()` calls where it wants to inspect, running the workflow, and calling `libretto resume --session <name>` to continue when it's ready. `pause()` is a no-op when `NODE_ENV === "production"`, so it's safe to leave in during development.
