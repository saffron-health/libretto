...
<Steps>
  ...
  <Step title="Log in when the browser opens (if needed)">
    ...
    libretto open https://sfbay.craigslist.org --headless --session one-shot-flow
    ```
  ...
  <Step title="The agent explores the page">
    The agent takes snapshots and tests selectors against the live DOM. You'll see it running commands like `libretto snapshot` and `libretto exec` in your terminal.
  ...
  <Step title="Check the validation run">
    ...
    ```bash  theme={null}
    # What the agent runs
    libretto run ./free-stuff.ts --headless
    ```
...
<Tip>
  After you log in, tell the agent to save the session with `libretto save`.
...
<Note>
  ...
  Run `npm create libretto@latest` or `libretto setup` in your project root
