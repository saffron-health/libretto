...
Recording URLs are temporary. If a URL expires, call this route again with the same `job_id` or `session_id` to generate a fresh URL for the existing recording. The recording itself is not regenerated or deleted when a URL expires. Recordings remain available for a limited time after the run. See [Data retention](/libretto-cloud-hosting/data-retention).
