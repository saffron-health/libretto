...
Auth: pass `apiKey` from a workflow credential (same locally and on hosted Cloud). Declare `libretto_api_key` in `credentials`, store it in the dashboard (or `LIBRETTO_CLOUD_LIBRETTO_API_KEY` locally), and pass `apiKey: input.credentials.libretto_api_key`.
...
export default workflow("portal-login", {
  ...
  async handler(ctx, input) {
    ...
    // Inbox is locked. otp.phoneNumber is the claimed E.164 number.
