...
Interactive login, logout, invites, API-key issuance, and member management are handled through the Libretto Cloud CLI auth flow rather than separate public `/v1/*` endpoints here. Use [Libretto Cloud authentication](/libretto-cloud-hosting/authentication) for that workflow.
