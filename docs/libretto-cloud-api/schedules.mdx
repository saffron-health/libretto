...
Prefer declaring `startUrl`, `gpu`, and `viewport` on the workflow definition so schedule fires inherit them. Use the schedule-level fields only for one-off overrides.
