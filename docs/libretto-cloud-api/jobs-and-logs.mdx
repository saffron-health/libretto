...
Prefer declaring `startUrl`, `gpu`, and `viewport` on the workflow definition so jobs inherit them from deploy metadata. Use the job-level fields only when you need a one-off override.
