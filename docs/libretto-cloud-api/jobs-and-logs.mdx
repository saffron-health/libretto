...
```bash
libretto cloud jobs create check-eligibility --params '{"member_id":"123"}'
libretto cloud jobs status <job-id>
libretto cloud jobs status <job-id> --watch
```

Use `libretto cloud catalogue` instead when discovering or running a
...
Job results stay for the life of your workspace, while logs age out sooner. See [Data retention](/libretto-cloud-hosting/data-retention).
