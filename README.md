...
The published `libretto` package lives in `packages/libretto` inside this monorepo.
...
- `packages/libretto` - published package, runtime, CLI, and tests
- `benchmarks/` - benchmark suite importing from `packages/libretto/src/`
- `evals/` - eval suite
- `apps/website` - website app

Within `packages/libretto`:
...
To check that skill mirrors are in sync without fixing them, run `pnpm --filter libretto check:skills`. To release, run `pnpm prepare-release`.
