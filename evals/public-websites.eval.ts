...
function agentInstruction(agentName: EvalAgentName, browserProvider: string): string {
  if (agentName === "browser-use") {
    const providerDescription =
      browserProvider === "kernel"
        ? "the configured Kernel browser provider"
        : browserProvider === "local"
          ? "its local browser"
          : `the configured ${browserProvider} browser provider`;
    return [
    ...
    browserProvider === "kernel"
      ...
      : browserProvider === "browserbase"
        ...
        : browserProvider === "steel"
          ? "Steel"
          : browserProvider === "libretto-cloud"
            ? "Libretto Cloud"
            : "local";
  const otherProviders = ["local", "Kernel", "browserbase", "Steel", "Libretto Cloud"]
    .filter((name) => name !== providerName)
    .join(", ");

  if (browserProvider === "kernel") {
    return [
  ...
  return [
    `Use Libretto with the configured ${providerName} browser provider; do not switch to ${otherProviders}, search-result snippets, mirrors, APIs, cached copies, or unrelated fallback sources.`,
    `${providerName} is not configured here with CAPTCHA solving.`,
    `If you see a CAPTCHA, challenge, bot-check, human-verification page, Access Denied, permission denied, blocked, 403/Forbidden, unusual traffic, or bot-detection page, report that the task failed immediately because ${providerName} cannot solve it in this eval.`,
  ].join(" ");
}
      ...
      `${websiteEval.task}. ${agentInstruction(
        ...
        agent.name === "libretto"
          ? [
              ...
              "The workflow should default-export `workflow(name, async (ctx) => ...)`, open the requested live site with the configured browser provider, perform the task, and return the final answer as structured data.",
              "Use Zod schemas when useful; the eval workspace includes `zod` as a direct dependency.",
