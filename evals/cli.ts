...
async function runCloudQuery(options: CloudQueryCliOptions): Promise<number> {
  ...
  if (options.command === "status") {
    ...
    try {
      progress = await readExecutionProgress(manifest.executionName);
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      process.stderr.write(
    ...
    const lines = [
      ...
      progress?.logUri ? `Logs: ${progress.logUri}` : null,
      "",
      "Uploaded results:",
      ...
      progress ? "" : "Cloud Run progress unavailable.",
      progress ? "Cloud Run:" : null,
      progress ? `  status: ${progress.status}` : null,
      progress ? `  running: ${progress.runningCount}` : null,
      progress ? `  pending: ${progress.pendingCount}` : null,
      progress ? `  succeeded: ${progress.succeededCount}` : null,
      progress ? `  failed: ${progress.failedCount}` : null,
      progress ? `  cancelled: ${progress.cancelledCount}` : null,
      progress ? `  retried: ${progress.retriedCount}` : null,
