...
Currently configured project: `mighty-lagoon` (target: `..`).
...
3. Open the parent repo in your coding agent (Claude Code, Cursor, …)
   and have it follow `data/mighty-lagoon/SETUP.md` to fill in
   `data/mighty-lagoon/INFO.md`.
...
`--project-id mighty-lagoon` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/mighty-lagoon/`.
