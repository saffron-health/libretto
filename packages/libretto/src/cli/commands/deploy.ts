...
export const deployInput = SimpleCLI.input({
  ...
  named: {
    name: SimpleCLI.option(deploymentNameSchema.optional(), {
      help: "Deployment name (optional; autogenerated if omitted)",
    }),
...
export const deployCommand = SimpleCLI.command({
  ...
  .handle(async ({ input }) => {
    ...
    const deploymentName = input.name ?? generateDeploymentName();

    if (!input.name) {
      console.log(`Using autogenerated deployment name: ${deploymentName}`);
    }
