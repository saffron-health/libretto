...
### Phase 7: Verification and follow-up scope ✅

Verified that daemon-backed `exec` preserves Playwright aria refs generated by a previous `exec` call on the same persistent page. `snapshot → exec` aria-ref generation remains future work because the current snapshot command captures screenshot/HTML artifacts and does not invoke Playwright's AI snapshot ref-generation path.

- [x] Add a regression test showing an aria ref generated by one daemon-backed `exec` call can be used by a later daemon-backed `exec` call in the same session.
- [x] Add regression coverage for daemon-backed `exec` and `readonly-exec` preserving console output when user code throws.
- [x] Run `pnpm -s test --filter=libretto -- daemon-ipc.spec.ts` — 10 passed.
- [x] Run `pnpm -s type-check --filter=libretto` — passes.
